...
std::unordered_map<int, DeepLearning_AI_obb*> g_mapDLE_AI_obb;
...
namespace
{
	thread_local bool obbAiLoggingEnabled = false;

	void ensureObbAiLoggerInitialized()
	{
		if (!obbAiLoggingEnabled)
			return;

		std::call_once(g_aiLoggerInitFlag, []()
			{
				char logPath[] = "d:\\debug\\log";
				g_log.initLog(logPath, "DeepLearning_AI_obb.log");
				g_log.Open(logPath, "DeepLearning_AI_obb.log");
			});
	}
...
void DeepLearning_AI_obb::configureAiLoggingForRequest(const char* xmlIn, const std::string& apiName)
{
	...
	obbAiLoggingEnabled = debugProcess == 1 || debugResult == 1;
}

bool DeepLearning_AI_obb::addAiLog(long logTypeCode, const char* content)
{
	if (!obbAiLoggingEnabled || content == nullptr)
		return false;

	ensureObbAiLoggerInitialized();
...
namespace
{
	...
	cv::Mat renderObbImage(const cv::Mat& srcImg, const std::vector<cv::RotatedRect>& rects, const std::vector<int>& categories);
	...
	void beginProcessXml(CMarkup& xml, DeepLearning_AI_obb* deepAI, const std::string& apiName, const char* moduleStatusCh);
	...
	DeepLearning_AI_obb* findDeepAIByModuleXml(char* xmlIn, const std::string& apiName)
	{
		ResolveXml rXml;
		int id = rXml.parseXmlInt(xmlIn, "Param", apiName, "moduleID");
		auto mapIter = g_mapDLE_AI_obb.find(id);
		if (mapIter == g_mapDLE_AI_obb.end())
			return nullptr;
	...
	void prepareProcessXmlInput(DeepLearning_AI_obb* deepAI, void* inImg, char* xmlIn, const std::string& apiName)
	{
	...
	void saveDebugSourceImage(DeepLearning_AI_obb* deepAI)
	{
	...
	cv::Mat cropByPreparedRectRoi(DeepLearning_AI_obb* deepAI, cv::Rect& rectSrc)
	{
	...
	void logWholeProcessCost(DWORD startTick)
	{
		...
		DeepLearning_AI_obb::addAiLog(LOG_MESSAGE, str.c_str());
	}
...
bool DeepLearning_AI_obb::isOnnxModelPath(const std::string& module_path)
{
...
bool DeepLearning_AI_obb::isTorchScriptModelPath(const std::string& module_path)
{
	const std::string extension = toLowerCopy(std::filesystem::path(module_path).extension().string());
...
// 根据模型文件扩展名选择具体推理后端
bool DeepLearning_AI_obb::inferModelRuntimeFormat(const std::string& module_path, ModelRuntimeFormat& runtime_format)
{
...
void DeepLearning_AI_obb::resetRuntimeState()
{
...
int DeepLearning_AI_obb::loadOnnxModule()
{
	m_ortEnv = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "DeepLearning_AI_obb");
...
void DeepLearning_AI_obb::resetOnnxRuntimeState()
{
	m_ortSession.reset();
	m_ortEnv.reset();
	m_ortInputNames.clear();
...
// 返回值：0=成功，非 0=失败
// ============================================================================
int DeepLearning_AI_obb::initObbAiObj(char* xmlIn, char** xmlOut, string apiName)
{
	int ret = 0;
	int id = 0;
	std::string modulePath;

	DeepLearning_AI_obb* deepAI = nullptr;

	id = parseInitObbXml(xmlIn, apiName);
	...
	{
		deepAI = new DeepLearning_AI_obb;
		g_mapDLE_AI_obb.insert(std::pair<int, DeepLearning_AI_obb*>(id, deepAI));
	}
...
int DeepLearning_AI_obb::parseInitObbXml(char* xmlIn, std::string apiName)
{
	int ret = 0;
	ResolveXml rXml;
...
int DeepLearning_AI_obb::outputInitXml(char* xmlOut, string apiName)
{
	// 构建 XML 字符串
	CMarkup xml;
	xml.SetDoc("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
...
//   2. 在全局 map 中查找并删除 DeepLearning_AI_obb 实例
//   3. 调用 outputDesXml 输出销毁状态 XML
// 返回值：0=成功，-4=模型未找到
// ============================================================================
int DeepLearning_AI_obb::destoryObbObject(char* xmlIn, char** xmlOut, std::string apiName)
{
	DeepLearning_AI_obb* deepAI = nullptr;
	ResolveXml rXml;
	int id = rXml.parseXmlInt(xmlIn, "Param", apiName, "moduleID");
	...
	// 释放模型对象内存
	// 注意：此处未从 g_mapDLE_AI_obb 中 erase 该条目，后续访问会产生悬空指针；
	// 实际使用时需确保 destory 后不再引用该 moduleID
	deepAI->outputDesXml(*xmlOut, apiName);
	delete deepAI;
	g_mapDLE_AI_obb.erase(mapIter);
...
void DeepLearning_AI_obb::outputDesXml(char* xmlOut, std::string apiName)
{
	// 构建 XML 字符串
	CMarkup xml;
	xml.SetDoc("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
...
// ============================================================================
int DeepLearning_AI_obb::loadModule(char* xmlIn, string apiName)
{
	try
	{
		int ret = 0;
		...
		// 没有显式传 inputSize 时，补OBB模型的默认输入尺寸
		if (m_inputSize == 0) {
			m_inputSize = 640;   // OBB
		}
		...
		DeepLearning_AI_obb::warmup(img);
	...
	catch (const std::exception& e)
	{
		...
		DeepLearning_AI_obb::addAiLog(LOG_ERROR, str.GetBuffer());
		m_pIsLoadedModule = false;
		m_pModStatus = false;
		return -1;
	}
...
void DeepLearning_AI_obb::warmup(at::Tensor img)
{
...
void DeepLearning_AI_obb::warmupOnnx(const torch::Tensor& img)
{
...
std::vector<torch::Tensor> DeepLearning_AI_obb::forwardOnnx(const torch::Tensor& tensor_img)
{
	if (!m_ortSession)
	{
...
int DeepLearning_AI_obb::processOneImgByObb(void* inImg, void*& outImg, char* xmlIn, char** xmlOut, std::string apiName)
{
	DeepLearning_AI_obb* deepAI = findDeepAIByModuleXml(xmlIn, apiName);
	if (deepAI == nullptr)
		return -4;
	try
	{
		...
		result = deepAI->runObb(cjImg, DC_VecRotaRect, DC_score, DC_class, DC_Pts, rectSrc); // 结果包含检测框、置信度分数和类别索引

		//deepAI->m_pDrcImg = renderObbImage(
		//	deepAI->m_pSrcImg,
		//	DC_VecRotaRect,
...
// ============================================================================
// OBB 旋转目标检测推理与后处理
// 与 runDetection 的关键区别：
//   - 模型输出直接是 Tensor（非 Tuple），格式 [batch, 特征维度, 检测数量]
//   - 特征维度 包含 [cx, cy, w, h, 类别置信度分数..., 角度(弧度)]
//   - 角度以弧度输出，需通过 RAD2DEG 转换为度数存到 RotatedRect.angle
//   - NMS 使用旋转框的包围矩形简化计算（因无原生旋转 NMS）
// 返回值：检测到的目标数量
// ============================================================================
int DeepLearning_AI_obb::runObb(cv::Mat& m_pSrcImg, std::vector<cv::RotatedRect>& DC_VecRotaRect, std::string& DC_score, std::string& DC_class, std::vector<cv::Point2f>& DC_Pts, cv::Rect& rectSrc) {
	...
	torch::Tensor det = buildObbDetections(outputs[0], conf_threshold, iou_threshold);
	int num_det = det.size(0);
	if (num_det == 0) return 0;

	appendObbDetections(det, pad_w, pad_h, scale, rectSrc, DC_VecRotaRect, DC_score, DC_class);
...
// OBB 后处理：
// 角度仍保留弧度，直到 appendObbDetections 再统一转成 OpenCV 使用的角度制
torch::Tensor DeepLearning_AI_obb::buildObbDetections(const torch::Tensor& output, float conf_threshold, float iou_threshold)
{
	auto pred = normalizeYoloOutput(output);

	int num_features = pred.size(1);
	int num_classes = num_features - 5;
	if (num_classes <= 0) {
		std::cout << "Invalid OBB output format" << std::endl;
		return torch::empty({ 0, 0 });
	}

	auto boxes_xywh = pred.slice(1, 0, 4);
	auto class_scores = pred.slice(1, 4, 4 + num_classes);
	auto angles = pred.slice(1, 4 + num_classes, 5 + num_classes).squeeze(1);

	auto max_tuple = torch::max(class_scores, 1);
	auto max_scores = std::get<0>(max_tuple);
	auto class_idxs = std::get<1>(max_tuple).to(torch::kFloat32);

	auto valid_indices = torch::nonzero(max_scores > conf_threshold).squeeze(1);
	if (valid_indices.size(0) == 0) {
		std::cout << "No detections above threshold" << std::endl;
		return torch::empty({ 0, 0 });
	}

	torch::Tensor dets = torch::cat({
		boxes_xywh.index_select(0, valid_indices),
		angles.index_select(0, valid_indices).unsqueeze(1),
		max_scores.index_select(0, valid_indices).unsqueeze(1),
		class_idxs.index_select(0, valid_indices).unsqueeze(1)
		}, 1).cpu();

	constexpr float RAD2DEG = 180.0f / CV_PI;
	torch::Tensor boxes_for_nms = torch::zeros({ dets.size(0), 4 });
	for (int idx = 0; idx < dets.size(0); ++idx) {
		float cx = dets[idx][0].item().toFloat();
		float cy = dets[idx][1].item().toFloat();
		float w = dets[idx][2].item().toFloat();
		float h = dets[idx][3].item().toFloat();
		float angle = dets[idx][4].item().toFloat() * RAD2DEG;

		cv::RotatedRect rRect(cv::Point2f(cx, cy), cv::Size2f(w, h), angle);
		cv::Rect2f bbox = rRect.boundingRect();
		boxes_for_nms[idx][0] = bbox.x;
		boxes_for_nms[idx][1] = bbox.y;
		boxes_for_nms[idx][2] = bbox.x + bbox.width;
		boxes_for_nms[idx][3] = bbox.y + bbox.height;
	}

	auto keep = nms_kernel(boxes_for_nms, dets.select(1, 5), iou_threshold);
	return torch::index_select(dets, 0, keep);
}

// 将首个 rotRectROI 转成普通矩形
// 若未配置 ROI，则默认整图参与推理
void DeepLearning_AI_obb::rotateRect2Rect()
{
...
DeepLearning_AI_obb::InferenceInput DeepLearning_AI_obb::prepareYoloInput(cv::Mat& image, int image_size)
{
	auto start = std::chrono::high_resolution_clock::now();
...
// 输出单阶段耗时，阶段名由调用方传入
void DeepLearning_AI_obb::logStageCost(const char* stage_name, std::chrono::high_resolution_clock::time_point start)
{
	auto end = std::chrono::high_resolution_clock::now();
...
// ============================================================================
std::vector<float> DeepLearning_AI_obb::LetterboxImage(const cv::Mat& src, cv::Mat& dst, const cv::Size& out_size) {
...
torch::Tensor DeepLearning_AI_obb::normalizeYoloOutput(torch::Tensor pred)
{
	if (pred.dim() == 3)
	{
		if (pred.size(0) == 1)
...
// 将 OBB 检测结果恢复到原图坐标，并把角度从弧度制转换为角度制
void DeepLearning_AI_obb::appendObbDetections(const torch::Tensor& det, float pad_w, float pad_h, float scale, const cv::Rect& rectSrc,
	std::vector<cv::RotatedRect>& boxes, std::string& scores, std::string& classes)
{
	constexpr float RAD2DEG = 180.0f / CV_PI;
	for (int index = 0; index < det.size(0); ++index) {
		cv::RotatedRect rect;
		rect.center.x = (det[index][0].item().toFloat() - pad_w) / scale + rectSrc.x;
		rect.center.y = (det[index][1].item().toFloat() - pad_h) / scale + rectSrc.y;
		rect.size.width = det[index][2].item().toFloat() / scale;
		rect.size.height = det[index][3].item().toFloat() / scale;
		rect.angle = det[index][4].item().toFloat() * RAD2DEG;
...
at::Tensor DeepLearning_AI_obb::nms_kernel(
...
int DeepLearning_AI_obb::parseProcessXml(void* inImg, char* xmlIn, string apiName)
{
	try
	{
		ResolveXml rXml;
...
std::vector<torch::Tensor> DeepLearning_AI_obb::forwardWithTiming(const torch::Tensor& tensor_img)
{
	auto start = std::chrono::high_resolution_clock::now();
