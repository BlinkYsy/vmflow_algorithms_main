...
// 全局模型对象映射表：moduleID → DeepLearning_AI_detect 实例指针
// DLL 内所有推理流程通过此表查找已注册的模型对象
std::unordered_map<int, DeepLearning_AI_detect*> g_mapDLE_AI_detect;

// 全局日志对象，用于记录所有操作日志
CFileLogEx g_log;
std::once_flag g_aiLoggerInitFlag;

namespace
{
	thread_local bool detectAiLoggingEnabled = false;

	void ensureDetectAiLoggerInitialized()
	{
		if (!detectAiLoggingEnabled)
			return;

		std::call_once(g_aiLoggerInitFlag, []()
			{
				char logPath[] = "d:\\debug\\log";
				g_log.initLog(logPath, "DeepLearning_AI_detect.log");
				g_log.Open(logPath, "DeepLearning_AI_detect.log");
			});
	}
	...
	std::string buildInputSizeMismatchHint(const std::string& message, int currentInputSize)
	{
		...
		hint.append(", but this TorchScript detect model appears to expect inputSize=");
...
void DeepLearning_AI_detect::configureAiLoggingForRequest(const char* xmlIn, const std::string& apiName)
{
	detectAiLoggingEnabled = false;
	if (xmlIn == nullptr)
		return;
	...
	detectAiLoggingEnabled = debugProcess == 1 || debugResult == 1;
}

bool DeepLearning_AI_detect::addAiLog(long logTypeCode, const char* content)
{
	if (!detectAiLoggingEnabled || content == nullptr)
		return false;

	ensureDetectAiLoggerInitialized();
...
namespace
{
	...
	void beginProcessXml(CMarkup& xml, DeepLearning_AI_detect* deepAI, const std::string& apiName, const char* moduleStatusCh);
	...
	cv::Mat renderDetectionImage(const cv::Mat& srcImg, const std::vector<cv::RotatedRect>& rects, const std::vector<int>& categories)
	{
	...
	DeepLearning_AI_detect* findDeepAIByModuleXml(char* xmlIn, const std::string& apiName)
	{
		ResolveXml rXml;
		int id = rXml.parseXmlInt(xmlIn, "Param", apiName, "moduleID");
		auto mapIter = g_mapDLE_AI_detect.find(id);
		if (mapIter == g_mapDLE_AI_detect.end())
			return nullptr;
	...
	void prepareProcessXmlInput(DeepLearning_AI_detect* deepAI, void* inImg, char* xmlIn, const std::string& apiName)
	{
	...
	// 输出一张带检测区域框的原始输入图，方便排查本次检测到底取了哪一块区域参与处理
	void saveDebugSourceImage(DeepLearning_AI_detect* deepAI)
	{
		if (!deepAI->m_pDebugProcess)
			return;

		cv::Mat buf = deepAI->m_pSrcImg.clone();
		//在复制图上画一个矩形框，位置就是 m_pRect，表示本次检测的 ROI 区域
	...
	cv::Mat cropByPreparedRectRoi(DeepLearning_AI_detect* deepAI, cv::Rect& rectSrc)
	{
		...
		const char* rectCh)
	{
		...
		addRenderXml(xml, rendPar, rectCh, false);
	...
	void logWholeProcessCost(DWORD startTick)
	{
		...
		DeepLearning_AI_detect::addAiLog(LOG_MESSAGE, str.c_str());
	}
	...
	void addScoreCategoryXml(CMarkup& xml, const std::string& score, const std::string& category, bool trimTrailingComma)
	{
		...
		const std::string categoryText = trimTrailingComma ? trimTrailingCommaOrDefault(category, "-1") : category;
...
bool DeepLearning_AI_detect::isOnnxModelPath(const std::string& module_path)
{
...
bool DeepLearning_AI_detect::isTorchScriptModelPath(const std::string& module_path)
{
	const std::string extension = toLowerCopy(std::filesystem::path(module_path).extension().string());
...
// 根据模型文件扩展名选择具体推理后端
bool DeepLearning_AI_detect::inferModelRuntimeFormat(const std::string& module_path, ModelRuntimeFormat& runtime_format)
{
...
void DeepLearning_AI_detect::resetRuntimeState()
{
...
int DeepLearning_AI_detect::loadOnnxModule()
{
	m_ortEnv = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "DeepLearning_AI_detect");
...
void DeepLearning_AI_detect::resetOnnxRuntimeState()
{
	m_ortSession.reset();
	m_ortEnv.reset();
	m_ortInputNames.clear();
...
// 返回值：0=成功，非 0=失败
// ============================================================================
int DeepLearning_AI_detect::initDetectAiObj(char* xmlIn, char** xmlOut, string apiName)
{
	int ret = 0;
	int id = 0;
	std::string modulePath;

	DeepLearning_AI_detect* deepAI = nullptr;

	id = parseInitDetectXml(xmlIn, apiName);
	...
	{
		deepAI = new DeepLearning_AI_detect;
		g_mapDLE_AI_detect.insert(std::pair<int, DeepLearning_AI_detect*>(id, deepAI));
	}
...
int DeepLearning_AI_detect::parseInitDetectXml(char* xmlIn, std::string apiName)
{
	int ret = 0;
	ResolveXml rXml;
...
int DeepLearning_AI_detect::outputInitXml(char* xmlOut, string apiName)
{
	// 构建 XML 字符串
	CMarkup xml;
	xml.SetDoc("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
...
// ============================================================================
// destoryObject — 销毁指定 moduleID 对应的模型实例
// 流程：
//   1. 解析 XML 获取 moduleID
//   2. 在全局 map 中查找并删除 DeepLearning_AI_detect 实例
//   3. 调用 outputDesXml 输出销毁状态 XML
// 注意：
// 返回值：0=成功，-4=模型未找到
// ============================================================================
int DeepLearning_AI_detect::destoryDetectObject(char* xmlIn, char** xmlOut, std::string apiName)
{
	DeepLearning_AI_detect* deepAI = nullptr;
	ResolveXml rXml;
	int id = rXml.parseXmlInt(xmlIn, "Param", apiName, "moduleID");
	...
	//找到了以后，取出这个键值对里的 value
	deepAI = mapIter->second;

	deepAI->m_pModStatus = false;
	// 释放模型对象内存
	// 注意：此处未从 g_mapDLE_AI_detect 中 erase 该条目，后续访问会产生悬空指针；
	// 实际使用时需确保 destory 后不再引用该 moduleID
	deepAI->outputDesXml(*xmlOut, apiName);
	delete deepAI;
	g_mapDLE_AI_detect.erase(mapIter);
	return 0;
...
void DeepLearning_AI_detect::outputDesXml(char* xmlOut, std::string apiName)
{
	// 构建 XML 字符串
	CMarkup xml;
	xml.SetDoc("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n");
	...
	xml.AddElem("moduStatus", initNumStr.GetBuffer());
	xml.SetAttrib("CH", "模块状态");
...
// ============================================================================
// loadModule — 解析初始化配置、加载模型后端，并完成预热
//
// 流程：
...
// ============================================================================
int DeepLearning_AI_detect::loadModule(char* xmlIn, string apiName)
{
	try
	{
		int ret = 0;
		...
		// 没有显式传 inputSize 时，补检测模型的默认输入尺寸
		if (m_inputSize == 0) {
			m_inputSize = 640; // 检测
		}
		...
		DeepLearning_AI_detect::warmup(img);
...
void DeepLearning_AI_detect::warmup(at::Tensor img)
{
...
void DeepLearning_AI_detect::warmupOnnx(const torch::Tensor& img)
{
...
std::vector<torch::Tensor> DeepLearning_AI_detect::forwardOnnx(const torch::Tensor& tensor_img)
{
	if (!m_ortSession)
	{
...
// ============================================================================
// 检测任务处理主流程：
//   1. 解析 XML 获取 moduleID 并查找 CDeepAI 实例
//   2. parseProcessXml + rotateRect2Rect：解析图像与 ROI
//   3. 根据 rotRectROI 裁剪图像区域（空则取全图）
//   4. 调用对应 run 方法执行推理 → 获得框/置信度分数/类别
//   5. 有结果：构建 XML（含 renderRectBox, score, category, 短/长轴, 角度）
//      无结果：构建 XML（defectStatus=1 正常，score=0, category=-100）
//   6. 计时并记录日志
// ============================================================================
int DeepLearning_AI_detect::processOneImgByDetection(void* inImg, void*& outImg, char* xmlIn, char** xmlOut, std::string apiName)
{
	DeepLearning_AI_detect* deepAI = findDeepAIByModuleXml(xmlIn, apiName);
	if (deepAI == nullptr)
		return -4;
	try
	{
		...
		// 添加边界保护
		//cv::Mat curImg = matIn(deepAI->m_pRect);
		...
		result = deepAI->runDetection(cjImg, DC_VecRotaRect, DC_score, DC_class, DC_Pts, rectSrc); // 结果包含检测框、置信度分数和类别索引

		//deepAI->m_pDrcImg = renderDetectionImage(
		//	deepAI->m_pSrcImg,
		//	DC_VecRotaRect,
		...
		{
			writeNormalResultXml(xmlOut, deepAI, apiName, DC_run, false, false, "shortAxis", "longAxis");
		}
	...
	catch (const std::exception& e)
	{
		...
		DeepLearning_AI_detect::addAiLog(LOG_ERROR, str.GetBuffer());
...
// 目标检测模型推理与后处理
// 后处理流程：
//   1. 预处理：Letterbox → 张量转换（BGR→RGB, /255, BCHW）
//   2. 推理：m_pModule.forward() → 取第一个元素作为检测输出
//   3. 置信度过滤：(cx,cy,w,h) → (x1,y1,x2,y2) → 得分×类别概率
//   4. NMS 去重 → 去 padding → 还原原图坐标
//   5. 将每个检测框的 4 个角点存入 DC_Pts，用于后续渲染
// 返回值：检测到的目标数量
// ============================================================================
int DeepLearning_AI_detect::runDetection(cv::Mat& m_pSrcImg, std::vector<cv::RotatedRect>& DC_VecRotaRect, std::string& DC_score, std::string& DC_class, std::vector<cv::Point2f>& DC_Pts, cv::Rect& rectSrc)
{

	float conf_threshold = m_pThreshold;
	...
	torch::Tensor det = buildDetectionDetections(outputs[0], conf_threshold, iou_threshold);
	...
	appendAxisAlignedDetections(det, pad_w, pad_h, scale, rectSrc, DC_VecRotaRect, DC_score, DC_class);
...
// 参数 inImg：输入的 OpenCV 图像（引用）
// 参数 outImg：输出的 cv::Mat* 强制转为 void*，调用方需以 delete cv::Mat* 释放
// 返回值：始终返回 0
int DeepLearning_AI_detect::outputDetectImg(cv::Mat inImg, void*& outImg)
{
	// 计算图像数据总字节数并分配独立内存块
...
// 常规检测后处理：先按 obj * cls 最大值做阈值过滤，再做 NMS
torch::Tensor DeepLearning_AI_detect::buildDetectionDetections(const torch::Tensor& output, float conf_threshold, float iou_threshold)
{
	auto pred = normalizeYoloOutput(output);

	torch::Tensor scores = pred.select(1, 4) * torch::amax(pred.slice(1, 5, pred.size(1)), 1);
	pred = pred.index({ scores > conf_threshold });
	if (pred.size(0) == 0) {
	...
	std::tuple<torch::Tensor, torch::Tensor> max_tuple = torch::max(pred.slice(1, 5, pred.size(1)), 1);
	pred.select(1, 4) = pred.select(1, 4) * std::get<0>(max_tuple);
	pred.select(1, 5) = std::get<1>(max_tuple);

	torch::Tensor dets = pred.slice(1, 0, 6).cpu();
	auto keep = nms_kernel(dets.slice(1, 0, 4), dets.select(1, 4), iou_threshold);
	return torch::index_select(dets, 0, keep);
}

// 将首个 rotRectROI旋转框 → 直立框转换
// 若未配置 ROI，则默认整图参与推理
void DeepLearning_AI_detect::rotateRect2Rect()
{
...
DeepLearning_AI_detect::InferenceInput DeepLearning_AI_detect::prepareYoloInput(cv::Mat& image, int image_size)
{
	auto start = std::chrono::high_resolution_clock::now();
...
// 输出单阶段耗时，阶段名由调用方传入
void DeepLearning_AI_detect::logStageCost(const char* stage_name, std::chrono::high_resolution_clock::time_point start)
{
	auto end = std::chrono::high_resolution_clock::now();
...
void DeepLearning_AI_detect::xywhToXyxyInPlace(torch::Tensor& pred)
{
...
// ============================================================================
std::vector<float> DeepLearning_AI_detect::LetterboxImage(const cv::Mat& src, cv::Mat& dst, const cv::Size& out_size) {
...
torch::Tensor DeepLearning_AI_detect::normalizeYoloOutput(torch::Tensor pred)
{
	if (pred.dim() == 3)
	{
		if (pred.size(0) == 1)
...
// 将常规检测结果写回输出容器
void DeepLearning_AI_detect::appendAxisAlignedDetections(const torch::Tensor& det, float pad_w, float pad_h, float scale, const cv::Rect& rectSrc,
...
// 将单个检测框从 letterbox 坐标系还原到原图坐标系，并包装成 OBB 旋转框
cv::RotatedRect DeepLearning_AI_detect::tensorBoxToRotatedRect(const torch::Tensor& det_row, float pad_w, float pad_h, float scale, const cv::Rect& rectSrc)
{
...
at::Tensor DeepLearning_AI_detect::nms_kernel(
...
int DeepLearning_AI_detect::parseProcessXml(void* inImg, char* xmlIn, string apiName)
{
	try
	{
		ResolveXml rXml;
...
std::vector<torch::Tensor> DeepLearning_AI_detect::forwardWithTiming(const torch::Tensor& tensor_img)
{
	auto start = std::chrono::high_resolution_clock::now();
