...
public:
	...
	float							m_pThreshold = 0.25;									// OBB定向检测时默认置信度阈值
...
public:
	...
	// 初始化或复用一个 DeepLearning_AI_obb 实例，并输出初始化结果 XML
	int initObbAiObj(char* xmlIn, char** xmlOut, std::string apiName);
	// 从初始化 XML 中解析 moduleID
	int parseInitObbXml(char* xmlIn, std::string apiName);
	// 执行单张图像的 OBB 旋转框检测任务
	int processOneImgByObb(void* inImg, void*& outImg, char* xmlIn, char** xmlOut, std::string apiName);
	// 按 moduleID 销毁实例并输出销毁结果 XML
	int destoryObbObject(char* xmlIn, char** xmlOut, std::string apiName);
	// 输出定向检测 OBB 图像结果
	int outputObbImg(cv::Mat inImg, void*& outImg);
	// OBB 旋转框检测推理，输出旋转框及分数/类别
	int runObb(cv::Mat& m_pSrcImg, std::vector<cv::RotatedRect>& DC_VecRotaRect, std::string& DC_score, std::string& DC_class, std::vector<cv::Point2f>& DC_Pts, cv::Rect& rectSrc);
	// 输出初始化结果 XML
...
private:
	...
	// OBB 检测后处理：角度保留、阈值过滤与 NMS
	torch::Tensor buildObbDetections(const torch::Tensor& output, float conf_threshold, float iou_threshold);
	// 将 OBB 检测结果批量写入输出容器
	void appendObbDetections(const torch::Tensor& det, float pad_w, float pad_h, float scale, const cv::Rect& rectSrc,
