...
本文档根据当前仓库源码整理，风格参考 `cpm-deepai-v2.0-v1.0.0.5` 的 README 与项目代码说明。目标是说明本项目的整体作用、目录结构、主要 DLL 接口、模型文件、构建运行方式以及常见调试入口。
...
| `DeepLearning_AI_detect.*` | 目标检测实现，支持 `.onnx`、`.torchscript`、`.pt` 模型路径。 |
...
| 字段 | 说明 |
| --- | --- |
| `modulePath` | 模型路径。AI 模块支持 `.onnx`、`.torchscript`、`.pt`；OCR 模块主要使用 `.onnx`。 |
