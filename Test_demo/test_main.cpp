...
namespace test_AI_common {
    ...
    std::vector<std::string> loadCategoryNamesForModel(const std::string& modulePath) {
        ...
        return loadCategoryNamesFromTorchScript(modulePath);
    }
    ...
    int parsePoseKptCountFromConfigText(const std::string& configText) {
        const std::regex kptShapePattern("\"kpt_shape\"\\s*:\\s*\\[\\s*(\\d+)\\s*,\\s*(\\d+)\\s*\\]");
        std::smatch match;
        if (!std::regex_search(configText, match, kptShapePattern)) {
            return 0;
        }

        const int kptCount = std::strtol(match[1].str().c_str(), nullptr, 10);
        const int kptDim = std::strtol(match[2].str().c_str(), nullptr, 10);
    ...
    int loadPoseKptCountFromTorchScript(const std::string& modulePath) {
        if (modulePath.empty()) {
            return 0;
        }
        ...
        try {
            ...
            return parsePoseKptCountFromConfigText(it->second);
        }
        catch (...) {
            return 0;
        }
...
namespace test_AI_detect {
    ...
    namespace {
        ...
        using test_AI_common::loadPoseKptCountFromTorchScript;
...
namespace test_AI_pose {
    ...
    namespace {
        ...
        void drawPoseDetections(
            ...
            for (size_t boxIndex = 0; boxIndex < boxes.size(); ++boxIndex) {
                ...
                for (size_t kpIndex = startIndex; kpIndex < endIndex; ++kpIndex) {
                    ...
                    if (!std::isfinite(kp.x) || !std::isfinite(kp.y) || kp.x < 0.0f || kp.y < 0.0f) {
                        continue;
                    }
        ...
        void savePoseVisualizationFromXml(
            ...
            //int keypointsPerBox = test_AI_common::loadPoseKptCountFromTorchScript(modulePath);
            //if (keypointsPerBox <= 0 && !keypoints.empty() && keypoints.size() % boxes.size() == 0) {
            //    keypointsPerBox = static_cast<int>(keypoints.size() / boxes.size());
            //}

            int keypointsPerBox = static_cast<int>(keypoints.size() / boxes.size());
        ...
        void handlePoseResult(
            ...
            savePoseVisualizationFromXml(inputImage, xmlResult, classNames, modulePath);
    ...
    int runTestDllPose() {
        ...
        <Param>
            <DLE_AI_pose_init>
                <modulePath>..\_model\obb+pose.onnx</modulePath>
        ...
        if (ret2 == 0) {
            handlePoseResult(inputImage, outImg, xmlOut, classNames, modulePath);
        }
        else {
